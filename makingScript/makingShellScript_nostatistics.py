...
folderName = "D://Data//CSS_data/real/amazon/" # network.dat
# =>
...
def printScript(_map, _aname, _pname) :
    ...
    if len(map.keys()) == 1 :
        for key in map.keys():
            for value in map[key]:
                param = "--nostiatistics True --algorithm "+_aname+ " --network "+_pname+ "network.dat --"+key+" "+str(value)
                script(param)
    ...
    if len(map.keys()) == 2 :
        ...
        for elem1 in v1:
            for elem2 in v2:
                param = "--nostiatistics True --algorithm "+_aname+ " --network "+_pname+ "/network.dat --" + keyList[0] + " " + str(elem1);
